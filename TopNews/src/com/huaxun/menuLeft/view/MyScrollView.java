package com.huaxun.menuLeft.view;
...
import com.huaxun.R;
import com.huaxun.menuLeft.ImageLoader;
...
public class MyScrollView extends ScrollView implements OnTouchListener {
	...
	public MyScrollView(Context context, AttributeSet attrs) {
		...
		setting = new Settings(context, false);
		if (setting.getPictureWallMode().equals(Settings.PICTURE_WALL_LOCAL)) {
			List<String> picInfos = ImageUtil.getLocalPictureInfos(context);		
			final int size =  picInfos.size();
			imageUrls = (String[])picInfos.toArray(new String[size]);
		} else {
			imageUrls = Images.sceneryImageUrls;
		}
	}
	...
	public void loadMoreImages() {
		if (hasSDCard()) {
			...
			if (startIndex < imageUrls.length) {
				Util.showToast(getContext(), "正在加载...");
				...
				Util.showToast(getContext(), "已没有更多图片");
			}
		} else {
			Util.showToast(getContext(), "未发现SD卡");
		}
	}
	...
	class LoadImageTask extends AsyncTask<String, Void, Bitmap> {
		 ...
		 * 
		 * @param imageUrl
		 *            图片的URL地址。(如果是加载本地图片，此处URL就是图片的本地存储路径)
		 * @return 图片的本地存储路径。
		 */
		private String getImagePath(String imageUrl) {
			...
			String imageDir = FileUtil.getPicturePath();
