package com.huaxun.chat.faceMode;
...
public class FaceConversionUtil {
	...
	public SpannableString addFace(Context context, int imgId, String spannableString) {
		...
		int pxValue = Util.dip2px(context, 24);
		bitmap = Bitmap.createScaledBitmap(bitmap, pxValue, pxValue, true);
	...
	private void dealExpression(Context context, SpannableString spannableString, Pattern patten, int start)
		...
		while (matcher.find()) {
			...
			if (resId != 0) {
				...
				bitmap = Bitmap.createScaledBitmap(bitmap, Util.dip2px(context, 60), Util.dip2px(context, 60), true);
	...
	private List<ChatEmoji> getData(int page) {
		...
		List<ChatEmoji> tempPageList = new ArrayList<ChatEmoji>();
		for(int i = startIndex; i < endIndex;i++) {
			tempPageList.add(emojis.get(i));
		}
		list.addAll(tempPageList);
