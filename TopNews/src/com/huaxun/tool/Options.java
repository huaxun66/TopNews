...
//				.showImageOnFail(R.drawable.small_image_holder_listpage)
				...
				// .delayBeforeLoading(int delayInMillis)// 设置图片下载前的延迟
				//.preProcessor(BitmapProcessor preProcessor)
				.resetViewBeforeLoading(true)// 设置图片在下载前是否重置，复位
				// .displayer(new RoundedBitmapDisplayer(20))//圆角角度
	...
	/** 圆角图片用到的图片加载配置 */
	public static DisplayImageOptions getRoundListOptions(int imageRes) {
		DisplayImageOptions options = new DisplayImageOptions.Builder()
		// // 设置图片在下载期间显示的图片
				...
				// .displayer(new FadeInBitmapDisplayer(20))// 淡入
				.build();
		return options;
	}
}
