 ...
 */
public class HotsBlogLayout extends RelativeLayout{
	...
	private static final String HOTS_PATH = NewsUrls.HOTS_BLOG;
	private boolean flag = false;
	private String nodeName = "热门博客";
	...
	public HotsBlogLayout(BlogListFragment blogListFragment) {
	...
	public HotsBlogLayout(BlogListFragment blogListFragment, AttributeSet attrs) {
	...
	public HotsBlogLayout(BlogListFragment blogListFragment, AttributeSet attrs, int defStyle) {
	...
	private void initView() {
		...
		hotsListView = (PullToRefreshListView) view.findViewById(R.id.pull_to_refresh_listview);
		adapter = new HotsBolgAdapter();
		hotsListView.setAdapter(adapter);		
		hotsListView.setMode(Mode.PULL_FROM_START);  //只能下拉刷新
		...
		hotsListView.setOnRefreshListener(new OnRefreshListener<ListView>() {
			...
			public void onRefresh(PullToRefreshBase<ListView> refreshView) {
				...
				if (refreshView.isHeaderShown()){
					hotsListView.getLoadingLayoutProxy().setRefreshingLabel("正在刷新...");
					hotsListView.getLoadingLayoutProxy().setPullLabel("下拉刷新");
					hotsListView.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
  	        	    refreshView.getLoadingLayoutProxy().setLastUpdatedLabel("最后更新时间:" + str);
  	        	    flag = false;
  	        	    loadHotsBlogInfo(true);
				}
			}
		});
		
		hotsListView.setOnLastItemVisibleListener(new OnLastItemVisibleListener() {
			@Override
			public void onLastItemVisible() {
					...
					if (footerView == null) {
						...
						hotsListView.getRefreshableView().addFooterView(footerView);  
				       }
			   }
		});
		
		//在Scroll或Fling的时候不要去加载图片
		//hotsListView.setOnScrollListener(new PauseOnScrollListener(ImageLoader.getInstance(), true, true));
		hotsListView.setOnScrollListener(new OnScrollListener() {
			@Override
			public void onScrollStateChanged(AbsListView view, int scrollState) {}
			@Override
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				blogListFragment.onScrollShowOrHide(firstVisibleItem, hotsListView.getRefreshableView());			
			}
		});
	}
	 ...
	 * 加载最新博客信息
	 */
	public void loadHotsBlogInfo(final boolean clean) {
		String url = HOTS_PATH;
	...
	private void stopRefresh() {
		hotsListView.postDelayed(new Runnable() {
			@Override
			public void run() {
				hotsListView.onRefreshComplete();
